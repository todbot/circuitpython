...
//|
//|         .. note:: Grain start position is currently deterministic (no randomization/jitter). A
//|            ``spread`` parameter for classic granular jitter may be added in a future release.
//|
...
static mp_obj_t audiodelays_granular_pitch_shift_make_new(const mp_obj_type_t *type, size_t n_args, size_t n_kw, const mp_obj_t *all_args) {
    enum { ARG_semitones, ARG_mix, ARG_grain_size, ARG_density, ARG_buffer_size, ARG_sample_rate, ARG_bits_per_sample, ARG_samples_signed, ARG_channel_count, };
